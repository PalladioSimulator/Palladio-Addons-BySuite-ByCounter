...
public class Main extends spec.benchmarks.compiler.MainBase {    
    ...
    public static void main(String[] args) throws Exception {//TODO clarify: how has added this?
        ...
        for(int i=0; i<_MK_numberOfRepetitions ; i++){
        	...
        	sum += _MK_val;
            System.err.println("MK "+i+": compiler.sunflow.Main.main: "+_MK_val+" ns");
        }
        Helper.plotDurationsHistory(_MK_meas, "measurement number", 
        		"duration [ns]", "Compiler.sunflow", "compiler.sunflow");
        System.err.println("MK: compiler.sunflow.Main.main 1st from "+_MK_numberOfRepetitions+" meas: "+_MK_meas.get(0));
        Collections.sort(_MK_meas);
        System.err.println("MK: compiler.sunflow.Main.main min from "+_MK_numberOfRepetitions+" meas: "+_MK_meas.get(0));
        System.err.println("MK: compiler.sunflow.Main.main med from "+_MK_numberOfRepetitions+" meas: "+_MK_meas.get(_MK_meas.size()/2));
        System.err.println("MK: compiler.sunflow.Main.main avg from "+_MK_numberOfRepetitions+" meas: "+sum/_MK_numberOfRepetitions);
        System.err.println("MK: compiler.sunflow.Main.main max from "+_MK_numberOfRepetitions+" meas: "+_MK_meas.get(_MK_meas.size()-1));
    }
