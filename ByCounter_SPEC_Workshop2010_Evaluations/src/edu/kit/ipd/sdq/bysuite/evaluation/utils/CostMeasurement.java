...
//		int numberOfRepetitions = 1;
		...
		if(prepareSuccess == false) {
			log.severe("ERROR: Could not initialize the measuring with " + 
...
//					diff = Math.round(((double)diff)/numberOfRepetitions);
	...
	private void measureAllClasses() {
		...
		this.methodnames.put(this.testClassName, methods);
		String className;
		for (Iterator<String> iter = this.methodnames.keySet().iterator(); iter.hasNext();) {
			className =  iter.next();
			this.measureOneClass(className, this.methodnames.get(className));
		}
	}
	...
	private void measureDurationOfInstrumentation_dynamic() {
		...
		this.instrumentationResults_dynamic = this.measure(measurableInstrumDyn, this.numberOfMeasurements);
	}
	...
	private void measureDurationOfInstrumentation_high() {
		...
		this.instrumentationResults_high = this.measure(measurableInstrumHigh, this.numberOfMeasurements);
	}
	...
	private void measureDurationOfInstrumentedMethods_dynamic() {
		...
		this.callCostInstrumResults_dynamic = this.measure(measurableRun, this.numberOfMeasurements);
	}
	...
	private void measureDurationOfInstrumentedMethods_high() {
		...
		this.callCostInstrumResults_high = this.measure(measurableRun, this.numberOfMeasurements);
	}
	...
	private MeasurableCostsForMethodRun measureDurationOfUninstrumentedMethod() {
		...
		this.callCostNoInstrumResults = this.measure(measurableRun, this.numberOfMeasurements);
