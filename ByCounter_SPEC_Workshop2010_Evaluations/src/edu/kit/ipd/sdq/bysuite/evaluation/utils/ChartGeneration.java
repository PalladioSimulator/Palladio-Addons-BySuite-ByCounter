...
public class ChartGeneration {
	...
	public ChartGeneration(int numberOfMeasurements) {
	...
	private void analyseData(HashMap<Integer, Integer> opcodeCounts) {
		...
		csvValues.append(this.paramForLogging + csvValueSeparator);
		...
		try {
			...
			// MK log.fine("Really found instructions: "+realColumns);
	...
	public void makeMLSetFromResult(IFullCountingResult r, int inputVarValue) {
		...
		long[] opcodeCounts = r.getOpcodeCounts();
		@SuppressWarnings("unused")
		SortedMap<String, Long> methodCallCounts = r.getMethodCallCounts();
		...
		try {
			...
			try {
				fosCSV.close();//TODO potential null pointer access
				fosSSV.close();//TODO potential null pointer access
			} catch (IOException e) {
				e.printStackTrace();
			}
		}
	...
	//				
	// default:
	// log.severe("ERROR: Unknown filetype " + filetype + ". Please use a
	// FILETYPE_* constant.");
	// }
	// } catch (IOException e) {
	// e.printStackTrace();
