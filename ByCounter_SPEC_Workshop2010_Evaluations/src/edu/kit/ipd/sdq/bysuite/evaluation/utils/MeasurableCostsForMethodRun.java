...
public class MeasurableCostsForMethodRun implements IMeasurable {
	...
	public void callForMeasuring() {
		// call the method
		MethodInvocationHelper.callMethodsNoArgs(log, 
				this.objInstance.getClass(), 
				this.objInstance, 
				this.methods);
	}
...
*/
		Class<?> classObj = null;
		try {
			if((this.instrumentationMode & CostMeasurement.INSTRUMENTATION_NONE) > 0) {
				classObj = ClassLoader.getSystemClassLoader().loadClass(this.method.getCanonicalClassName());
				log.fine("Declared methods: "+Arrays.toString(classObj.getDeclaredMethods()));
			} else {	// instrument the method
				InstrumentationParameters p = new InstrumentationParameters(this.methods);
				if((this.instrumentationMode & CostMeasurement.INSTRUMENTATION_USE_HIGH_REGISTERS) > 0) {
		...
		try {
			this.objInstance = classObj.newInstance(); //Problem in constructors with parameters...
		} catch (InstantiationException e1) {
			e1.printStackTrace();
			log.severe("Instantiation failure");
			return false;
		...
		if(this.objInstance == null) {
			log.severe("Error: Could not create class instance.");
			return false;
		}
