...
public class OldSimpleTestDriver {
	...
	public OldSimpleTestDriver() {	
		...
		log.fine("Operation from "+(new File(".")).getAbsolutePath());
	}
	...
	public final void start(String[] args) {
		if(args!=null && args.length==1){
			...
			if(param>=0 && param<11){
				...
				log.severe("Parameter out of range: "+param+" (should be between 0 and 10)");
			}
		}else{
			log.severe("Requires exactly one parameter to describe the input data, " +
...
//		OutputBuffer ob = null;
		log.fine("Compressing for option "+param+", i.e. file "+
...
//            byte[] result = new byte[length];
            ...
            log.fine((finish-start)+" ns to compute statistics");
...
//            ob = Compress.performAction(input, length, Compress.COMPRESS, result);
            ...
            log.fine(nsDuration+" ns="+usDuration+" us="+msDuration+" ms to compress "+length+" bytes to "+
