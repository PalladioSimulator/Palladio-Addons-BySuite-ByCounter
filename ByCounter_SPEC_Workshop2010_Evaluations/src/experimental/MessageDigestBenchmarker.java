...
//	private static final int DEFAULT_BYTEARRAY_SIZE = 100*1000;
	...
	private void createRandomInputData(
		...
		this.log.fine("Starting creating random data");
		for(int i=0; i<this.randomInputData.length; i++){
			this.rd.nextBytes(this.randomInputData[i]);
			System.out.print(".");
		}
		System.out.println(":");
		this.log.fine("Finised creating random data");
	}
	...
	private Long[] measureMessageDigesting(
		...
		this.log.fine("Entering with platfromDesc = "+platfromDesc+", "+
		...
		for(int j=0; j<randomDataUnitsIndexes.length; j++){
			randomDataUnitsIndexes[j] = this.rd.nextInt(nrOfDifferentRandomInputs);
		}
		this.createRandomInputData(inputSize,nrOfDifferentRandomInputs);

		for(int n = 0; n<nrOfMeasurements; n++){
			this.log.fine("Starting Measurement "+n);
			try {
				L1 = System.nanoTime();
...
//				System.out.println(":");
			...
			for(int j=0; j<this.randomInputData.length; j++){
				this.randomInputData[j][this.rd.nextInt(this.randomInputData[0].length)] = (new Integer(this.rd.nextInt(256))).byteValue();
			}
//			hashesHashes[i] = md.digest(hashes[rd.nextInt(nrOfChainings)]);
			//TODO only the last measurement is logged...
//			log.fine((L2-L1)+" ns for MD creation (incl. nanoTime())");
//			log.fine((L4-L3)+" ns for "/*+nrOfChainings*/+" MD update(s) (incl. nanoTime())");
////			log.info((L4-L3)/*/nrOfChainings*/+" ns for MD update (incl. nanoTime() share)");
//			log.fine((L6-L5)+" ns for MD digest (incl. nanoTime())");
//			log.fine(hashes);
		}
		Collections.sort(getInstanceMeasurements);
		Collections.sort(updateTotalMeasurements);
		Collections.sort(digestMeasurements);
		this.log.info(getInstanceMeasurements.get(0)+" ns: min of getInstance(...)");
		this.log.info(updateTotalMeasurements.get(0)+" ns: min of update(...)");
		this.log.info(digestMeasurements.get(0)+" ns: min of digest()");
		this.log.info(getInstanceMeasurements.get(getInstanceMeasurements.size()-1)+" ns: max of getInstance(...)");
		this.log.info(updateTotalMeasurements.get(updateTotalMeasurements.size()-1)+" ns: max of update(...)");
		this.log.info(digestMeasurements.get(digestMeasurements.size()-1)+" ns: max of digest()");
		...
		this.log.fine("======================================================");
		this.log.info(ret[0]+" ns: median of getInstance(...)");
		this.log.info(ret[1]+" ns: median of update(...)");
		this.log.info(ret[2]+" ns: median of digest()");
...
//		FileOutputStream fos = new FileOutputStream(masterFilePath);
		...
		try {
			...
			this.log.fine(""+masterFile.length());
