...
public class Whetstone extends Applet {//TODO MK add main method
	
	private static boolean _MK_executeOnce = true;

	private static boolean _MK_measureEvenIfExecutingOnce = false;
	
	private static int _MK_nrOfMeasurementsIfNotExecutedOnce = 2048;
	...
	public static void main(String[] args) {
		...
		int prm2_xtra = 10;
		int prm3_x100 = 100;
		int prm4_calibrate = 10;
		
		if(_MK_executeOnce){//how can we know that the method is not pure?
//			w.whetstones(prm2_xtra, prm3_x100, prm4_calibrate);
			if(_MK_measureEvenIfExecutingOnce){
			...
			double sum = 0D;
			for(int n=0; n<_MK_nrOfMeasurementsIfNotExecutedOnce; n++){
				long start = System.nanoTime();
				w.whetstones(prm2_xtra, prm3_x100, prm4_calibrate);
				long stopp = System.nanoTime();
			...
			long avg = new Double(sum/_MK_nrOfMeasurementsIfNotExecutedOnce).longValue();
			...
			System.out.println(
					...
					", x100="+prm3_x100+
					", calibrate="+prm4_calibrate+
					", nrOfMeasurements="+_MK_nrOfMeasurementsIfNotExecutedOnce+": \n"+
					"min="+min+", \nmed="+med+",\navg="+avg+",\nmax="+max);
		}
	}
	...
	public void displayAll(Graphics g) {
		...
		if (whatToDisplay == -1) {
			...
			g.drawString(
					"                                 WHETSTONE BENCHMARK", 5,
					yms);
			g.drawString(
					"     Before running this benchmark, ensure that all other applications",
					5, yms + 2 * yinc1);
			g.drawString(
					"     and windows are closed. The browser window should preferably be",
					5, yms + 3 * yinc1);
			g.drawString(
					"    maximized and, when running, the mouse should not be moved (much).",
					5, yms + 4 * yinc1);
			g.drawString(
					"                 Also, the window should not be resized or scrolled.",
					5, yms + 5 * yinc1);
			for (j = 0; j < 10; j++) {
				g.drawString(
						"        Click on the Run button to execute the Whetstone benchmark. ",
						5, yms + 7 * yinc1);
			}
		}
		if (whatToDisplay > -1) {
			...
			g.drawString("Copyright (C) 1997, Roy Longbottom ", 150,
					wheight - 5);
		...
		if (whatToDisplay > 1) {
			...
			g.drawString("   Graphics area copy at " + pixelrate
					+ " million pixels per second", xbox + 5, boxtop + boxhigh
					+ 40);
		}
		if (whatToDisplay > 2) {
			g.setFont(font5);
			...
			g.drawString("  Print Screen key, then paste"
					+ " the Clipboard contents into a document.", xbox + 3,
					boxtop + 30);
		}
	 ...
	 * @param g
	 */
	public void movedata(Graphics g) {//called from paing(G) only
		double time1 = dtime();
		int i;
	 ...
	 * @param t2
	 */
	public void p3(float t, float t1, float t2) {//called from whetstones(i,i,i)
		x = y;
		y = z;
		x = t * (x + y);
		y = t1 * (x + y);
	 ...
	 * @param t2
	 */
	public void pa(float e[], float t, float t2) {//called from whetstones(i,i,i)
		long j;
		for (j = 0; j < 6; j++) {
	 ...
	 */
	public void paint(Graphics g) {//too long for instrumentation
		int i, j;
		double howlong;
		...
		if (count == 0) {
			...
			for (routine = 1; routine < 9; routine++) {
				...
				g.setColor(Color.yellow);
				g.setFont(font3);
				g.drawString("Running time about " + duration + " seconds "
						+ xtra + " (x" + duration + ") passes", xbox + 20,
						boxtop + 14);
		...
		if (whatToDisplay == 1) {
			...
			pixelrate = (int) ((double) boxwide * (double) (boxwide - 1)
					* (double) boxhigh * (double) passcount / time2 / 1000000.0);
	 ...
	 * @param l
	 */
	public void po(float e1[], int j, int k, int l) {//called from whetstones(i,i,i)
		e1[j] = e1[k];
		e1[k] = e1[l];
		e1[l] = e1[j];
		return;
	 ...
	 * @param section
	 */
	public void pout(String title, float ops, int type, float checknum,//called from whetstones(i,i,i)
	...
	/**
	 * The routing field should be set to 1..8. 
	...
	public void whetstones(
			int xtra, 
			final int x100, //final per se
			final int calibrate//final per se
			) {
		...
		case 4:
			...
			timeb = dtime() - timea;
			x = e1[0] + e1[1];
			pout("N4 fixed point   \0", (float) (n4 * 15) * (float) (xtra), 2,
					x, timeb, calibrate, 4);
			break;
		...
		case 5:
			...
			timea = dtime();
			{
				for (ix = 0; ix < xtra; ix++) {
					for (i = 1; i < n5; i++) {
						x = (float) (t * Math.atan(t2 * Math.sin(x)
								* Math.cos(x)
								/ (Math.cos(x + y) + Math.cos(x - y) - 1.0)));
						y = (float) (t * Math.atan(t2 * Math.sin(y)
								* Math.cos(y)
								/ (Math.cos(x + y) + Math.cos(x - y) - 1.0)));
					}
					t = 1.0f - t;
			...
			timeb = dtime() - timea;
			pout("N5 sin,cos etc.  \0", (float) (n5 * 26) * (float) (xtra), 2,
					y, timeb, calibrate, 5);
			break;
		...
		case 6:
			...
			timeb = dtime() - timea;
			pout("N6 floating point\0", (float) (n6 * 6) * (float) (xtra), 1,
					z, timeb, calibrate, 6);
			break;
		...
		case 8:
			...
			timeb = dtime() - timea;
			pout("N8 exp,sqrt etc. ", (float) (n8 * 4) * (float) (xtra), 2, x,
					timeb, calibrate, 8);
			break;
		}
