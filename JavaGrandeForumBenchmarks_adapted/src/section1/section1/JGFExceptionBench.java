...
public class JGFExceptionBench implements JGFSection1 {
	...
	public void JGFrun() {
		...
		while (time < TARGETTIME && size < MAXSIZE) {
			...
			JGFInstrumentor.addOpsToTimer("Section1:Exception:Throw",
					(double) 8 * size);
		...
		while (time < TARGETTIME && size < MAXSIZE) {
			...
			JGFInstrumentor.addOpsToTimer("Section1:Exception:New", (double) 8
					* size);
		...
		while (time < TARGETTIME && size < MAXSIZE) {
			...
			JGFInstrumentor.addOpsToTimer("Section1:Exception:Method",
					(double) 8 * size);
